...
---
 ...
 15. Tối Ưu Context & "Think in Code" — Không Nhồi Dữ Liệu Thô

- Khi cần phân tích log, đếm file, quét lỗi, phân tích AST, trích xuất dữ liệu từ nhiều file: Viết script ngắn (Node.js/Python) xử lý tại chỗ và chỉ in ra kết quả (`stdout`), KHÔNG đọc hàng chục file dung lượng lớn vào context.
- Dùng công cụ `context-mode` (FTS5 search/sandbox) để giải phóng 98% dung lượng ngữ cảnh.
- Giữ cho cửa sổ ngữ cảnh luôn sạch để AI duy trì sự sắc bén, chính xác và không bị "mất trí nhớ" trong các phiên làm việc kéo dài.
